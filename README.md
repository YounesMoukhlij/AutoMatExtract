...
| `activation_energy.csv` | Every ion-migration energy-barrier value found — under any name the paper used for it (activation energy, Ea, migration/hopping/diffusion barrier, energy barrier) — one row per candidate, with `matched_as` recording which term matched, `ea_type` giving the kind of Ea (`bulk`, `grain boundary`, `surface`, `total`, or `unspecified` when the text names none — judged from the qualifier nearest the value in the sentence, or from the column header / row label / caption for tables; also present in `properties.csv`, `relations.csv`, the JSON and the summary CSV as `*_Type`), material (when resolved), value, unit, confidence, page/section/sentence. A focused view so this commonly-needed property doesn't have to be filtered out of `properties.csv` |
